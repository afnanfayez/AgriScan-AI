...
const staticCommentsFallback: Record<string, any[]> = {
  'post-static-2': [
    {
      id: 'cmt-static-1',
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ success: true, posts: staticPostsFallback });
    }
  ...
  try {
    ...
    if (body.action === 'like') {
      ...
      if (updateError || !updatedPost) {
        ...
        return NextResponse.json({ error: 'Failed to update likes' }, { status: 400 });
      }

      return NextResponse.json({ success: true, post: mapPost(updatedPost) });
    }
    ...
    if (body.action === 'comment') {
      ...
      if (commentError || !newComment) {
        ...
        return NextResponse.json({ error: commentError?.message || 'Failed to post comment' }, { status: 400 });
      }

      return NextResponse.json({ success: true, comment: mapComment(newComment) });
    }
