...
import { getSupabaseServerClient, uploadImageToStorage } from '@/lib/supabase';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (scanError || !newScan) {
      ...
      return NextResponse.json({ error: scanError?.message || 'Failed to save scan record' }, { status: 400 });
    }
    ...
    // Map responses to match existing frontend shapes
    const mappedScan = {
      id: newScan.id,
      plantId: newScan.plant_id,
      userId: newScan.user_id,
      imageUrl: newScan.image_url,
      diagnosis: newScan.diagnosis,
      confidence: newScan.confidence,
      severity: newScan.severity,
      symptoms: newScan.symptoms,
      createdAt: newScan.created_at,
    };

    const mappedTreatment = newTreatment ? {
      id: newTreatment.id,
      scanId: newTreatment.scan_id,
      plantId: newTreatment.plant_id,
      userId: newTreatment.user_id,
      type: newTreatment.type,
      organicSteps: newTreatment.organic_steps,
      chemicalSteps: newTreatment.chemical_steps,
      status: newTreatment.status,
      createdAt: newTreatment.created_at,
      resolvedAt: newTreatment.resolved_at,
    } : null;
